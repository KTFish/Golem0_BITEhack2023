...
from sklearn.metrics import recall_score, r2_score, explained_variance_score, mean_squared_error, mean_poisson_deviance, mean_gamma_deviance, mean_absolute_percentage_error, d2_tweedie_score
...
class SunlightModel():
    ...
    def train_model(self, X, Y):
        ...
        rf = RandomForestRegressor(
            max_features='sqrt', max_depth=11, bootstrap=False, n_estimators=400)
...
def generate():
    ...
    X = np.load(PATH_X)
    rf = SunlightModel()
    rf = rf.load_model(PATH_MODEL)

    rnd = random.randrange(DAYS, len(X))
    x_predict = X[rnd-DAYS:rnd]
    print(rf.predict(x_predict)[12])
    return rf.predict(x_predict)[12]*100
